...
module Version : sig type t = { major : int; minor : int; patch : int; } end
...
module AudioBuffer : sig type t end
module AudioDeviceID : sig type t end
module AudioSpec : sig type t end
module AudioStream : sig type t end
module AudioFormat :
  sig
    type t =
        AUDIO_U8
...
external load_wav :
  filename:string -> spec:AudioSpec.t -> AudioBuffer.t * int32
  = "caml_SDL_LoadWAV"
...
external map_RGB : PixelFormat.allocated -> rgb:rgb -> int32
  = "caml_SDL_MapRGB"
external map_RGBA : PixelFormat.allocated -> rgba:rgba -> int32
  = "caml_SDL_MapRGBA"
...
module Surface : sig type t end
module SurfaceBigarray : sig type t = Surface.t end
external create_rgb_surface :
  width:int -> height:int -> depth:int -> Surface.t
...
external fill_rect : dst:Surface.t -> rect:Rect.t -> color:int32 -> unit
  = "caml_SDL_FillRect"
external blit_surface :
  src:Surface.t ->
  src_rect:Rect.t -> dst:Surface.t -> dst_rect:Rect.t -> Rect.t
  = "caml_SDL_BlitSurface"
external blit_surf :
  src:Surface.t -> dst:Surface.t -> dst_rect:Rect.t -> Rect.t
  = "caml_SDL_BlitSurf"
external blit_surfs :
  src:Surface.t -> dst:Surface.t -> dst_rect:Rect.t -> unit
  = "caml_SDL_BlitSurfs"
external surface_blit_pixels_unsafe : Surface.t -> string -> unit
...
external set_color_key_map_rgb :
  Surface.t -> enable:bool -> rgb:int * int * int -> unit
...
external create_rgb_surface_from :
  pixels:(int, Bigarray.int8_unsigned_elt, Bigarray.c_layout)
         Bigarray.Array1.t ->
  width:int ->
  height:int ->
  depth:int ->
...
module WindowPos : sig type t = [ `centered | `pos of int | `undefined ] end
...
module GLcontext : sig type t end
external gl_create_context : win:Window.t -> GLcontext.t
  = "caml_SDL_GL_CreateContext"
external gl_make_current : win:Window.t -> ctx:GLcontext.t -> int
...
external create_window :
  ...
  dims:int * int -> flags:WindowFlags.t list -> Window.t
  = "caml_SDL_CreateWindow"
external create_window_2 :
  title:string ->
  x:WindowPos.t ->
  y:WindowPos.t ->
  width:int -> height:int -> flags:WindowFlags.t list -> Window.t
...
external get_window_surface : Window.t -> Surface.t
...
module Renderer : sig type t end
module RendererInfo :
  sig
    type t = {
      name : string;
...
module RendererFlags :
  sig
    type t = Software | Accelerated | PresentVSync | TargetTexture
    val to_string : t -> string
    val of_string : string -> t
  end
module RendererFlip :
  sig type t = Flip_None | Flip_Horizontal | Flip_Vertical end
...
module Texture : sig type t end
external create_window_and_renderer :
  width:int ->
  height:int -> flags:WindowFlags.t list -> Window.t * Renderer.t
  = "caml_SDL_CreateWindowAndRenderer"
external create_renderer :
  win:Window.t -> index:int -> flags:RendererFlags.t list -> Renderer.t
...
external render_set_logical_size2 :
  Renderer.t -> width:int -> height:int -> unit
  = "caml_SDL_RenderSetLogicalSize2"
external render_set_viewport : Renderer.t -> Rect.t -> unit
  = "caml_SDL_RenderSetViewport"
external render_set_clip_rect : Renderer.t -> Rect.t -> unit
...
external render_draw_point : Renderer.t -> int * int -> unit
  = "caml_SDL_RenderDrawPoint"
external render_draw_point2 : Renderer.t -> x:int -> y:int -> unit
  = "caml_SDL_RenderDrawPoint2"
external render_draw_points : Renderer.t -> points:(int * int) array -> unit
  = "caml_SDL_RenderDrawPoints"
external render_draw_line : Renderer.t -> (int * int) * (int * int) -> unit
  = "caml_SDL_RenderDrawLine"
external render_draw_line2 :
  Renderer.t -> p1:int * int -> p2:int * int -> unit
  = "caml_SDL_RenderDrawLine2"
external render_draw_lines : Renderer.t -> (int * int) array -> unit
  = "caml_SDL_RenderDrawLines"
external render_draw_rect : Renderer.t -> Rect.t -> unit
  = "caml_SDL_RenderDrawRect"
external render_draw_rects : Renderer.t -> Rect.t array -> unit
  = "caml_SDL_RenderDrawRects"
external render_fill_rect : Renderer.t -> Rect.t -> unit
  = "caml_SDL_RenderFillRect"
external render_fill_rects : Renderer.t -> Rect.t array -> unit
  = "caml_SDL_RenderFillRects"
external render_copy :
  Renderer.t ->
  texture:Texture.t -> ?src_rect:Rect.t -> ?dst_rect:Rect.t -> unit -> unit
  = "caml_SDL_RenderCopy"
external render_copyEx :
  ...
  ?src_rect:Rect.t ->
  ?dst_rect:Rect.t ->
  ?angle:float -> ?center:int * int -> ?flip:RendererFlip.t -> unit -> unit
...
external set_texture_color_mod3 :
  Texture.t -> r:int -> g:int -> b:int -> unit
...
module HapticPosition :
  sig
    type t = { left : bool; right : bool; up : bool; down : bool; }
    val string_of_pos : t -> string
  end
module HapticDirection :
  sig
    type t =
        Centered
      | Up
...
val from_input :
  [< `Buffer of bytes | `Filename of string | `String of string ] -> RWops.t
val from_input_opt :
  [> `Buffer of bytes | `Filename of string | `String of string ] ->
  RWops.t option
...
module KeyState : sig type t = SDL_RELEASED | SDL_PRESSED val to_string : t -> string end

(** {2:joy Joystick Support} *)

module Joystick : sig type t end
...
module MousePosition : sig type t = int * int end
...
module JoyDeviceEvent :
  sig
    type event_type = SDL_JOYDEVICEADDED | SDL_JOYDEVICEREMOVED
...
module QuitEvent : sig type t = { quit_timestamp : int32; } end
module TextEditingEvent :
  sig
    type t = {
      te_timestamp : int32;
      te_window_ID : int32;
